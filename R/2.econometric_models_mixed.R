...
cat("=== ECONOMETRIC MODELS: PENSION SUSTAINABILITY (FIRST DIFFERENCES) ===\n\n")
...
if(nrow(model1_data) > 20) {
  ...
  # Pooled OLS
  model1_pooled <- plm(d_Pension_GDP ~ d_FLFP + CPR + TFR + d_GDP_per_capita + 
  ...
  model1_re <- try(plm(d_Pension_GDP ~ d_FLFP + CPR + TFR + d_GDP_per_capita + 
                       d_Life_expectancy_65 + d_Female_tertiary_education + d_Urban_rate,
...
if(nrow(model2_data) > 20) {
  ...
  # Pooled OLS
  model2_pooled <- plm(d_Old_age_dependency ~ d_FLFP + CPR + TFR + d_GDP_per_capita + 
                       d_Life_expectancy_65 + d_Female_tertiary_education + d_Urban_rate,
  ...
  # Random Effects (with error handling)
  model2_re <- try(plm(d_Old_age_dependency ~ d_FLFP + CPR + TFR + d_GDP_per_capita + 
                       d_Life_expectancy_65 + d_Female_tertiary_education + d_Urban_rate,
...
if(nrow(model3_data) > 20) {
  ...
  # Pooled OLS
  model3_pooled <- plm(d_Pension_financing_gap ~ d_FLFP + CPR + TFR + d_GDP_per_capita + 
                       d_Life_expectancy_65 + d_Female_tertiary_education + d_Urban_rate,
  ...
  # Random Effects (with error handling)
  model3_re <- try(plm(d_Pension_financing_gap ~ d_FLFP + CPR + TFR + d_GDP_per_capita + 
                       d_Life_expectancy_65 + d_Female_tertiary_education + d_Urban_rate,
...
if(nrow(model4_data) > 20) {
  ...
  # Pooled OLS
  model4_pooled <- plm(d_Social_security_GDP ~ d_FLFP + CPR + TFR + d_GDP_per_capita + 
                       d_Life_expectancy_65 + d_Female_tertiary_education + d_Urban_rate,
  ...
  # Random Effects (with error handling)
  model4_re <- try(plm(d_Social_security_GDP ~ d_FLFP + CPR + TFR + d_GDP_per_capita + 
                       d_Life_expectancy_65 + d_Female_tertiary_education + d_Urban_rate,
...
if(length(selected_models) > 0) {
  ...
  stargazer(selected_models,
            type = "text",
            title = "Panel Data Models: Pension Sustainability (First Differences with Model Selection)",
            ...
            covariate.labels = c("Δ Female Labor Participation",
                               ...
                               "Δ Urban Rate"),
            out = "R/econometric_mixed_results_table.txt")
  
  cat("Results table saved to: R/econometric_mixed_results_table.txt\n")
  cat("Model objects saved to: econometric_models_mixed_results.RData\n")
}

cat("\n=== SUMMARY OF FIRST DIFFERENCES APPROACH ===\n")
