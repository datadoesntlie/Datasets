...
cat("=== DIAGNOSTIC TESTS FOR LEVELS METHODOLOGY ===\n\n")
...
diagnostic_results_levels <- list()
...
for (dep_var in dependent_vars) {
  ...
  formula_str <- paste(dep_var, "~ FLFP + CPR + TFR + GDP_per_capita + Life_expectancy_65 + Female_tertiary_education + Urban_rate + Year_centered + Year_squared")
  ...
  complete_data <- model_data[complete.cases(model_data[, c("FLFP", "CPR", "TFR", 
                                                           "GDP_per_capita", "Life_expectancy_65", 
                                                           "Female_tertiary_education", "Urban_rate",
                                                           "Year_centered", "Year_squared")]), ]
  
  if(nrow(complete_data) > 20) {
    corr_matrix <- cor(complete_data[, c("FLFP", "CPR", "TFR", "GDP_per_capita", 
                                        "Life_expectancy_65", "Female_tertiary_education", 
                                        "Urban_rate", "Year_centered", "Year_squared")], use = "complete.obs")
  ...
  diagnostic_results_levels[[dep_var]] <- model_tests
  
  cat("\n", rep("=", 80), "\n")
}

cat("\n=== DIAGNOSTIC SUMMARY FOR LEVELS APPROACH ===\n")

summary_df_levels <- data.frame(
  Model = character(),
...
for (dep_var in names(diagnostic_results_levels)) {
  tests <- diagnostic_results_levels[[dep_var]]
  ...
  summary_df_levels <- rbind(summary_df_levels, data.frame(
    Model = dep_var,
...
print(summary_df_levels)

write_csv(summary_df_levels, "R/4.diagnostic_summary_levels.csv")
save(diagnostic_results_levels, file = "R/4.diagnostic_results_levels.RData")

cat("\n=== DETAILED DIAGNOSTIC TEST RESULTS (LEVELS) ===\n")

detailed_results_levels <- data.frame(
  Model = character(),
  Test_Type = character(),
  Test_Name = character(),
  Statistic = numeric(),
...
for (model_name in names(diagnostic_results_levels)) {
  tests <- diagnostic_results_levels[[model_name]]
  ...
  if("hausman" %in% names(tests)) {
    ...
    detailed_results_levels <- rbind(detailed_results_levels, data.frame(
      Model = model_name,
...
print(detailed_results_levels)
write_csv(detailed_results_levels, "R/4.detailed_diagnostic_results_levels.csv")

cat("\n=== LEVELS METHODOLOGY RECOMMENDATIONS ===\n")
...
cat("4. Monitor multicollinearity issues, especially with time trends\n")
cat("5. Results saved to: 4.diagnostic_summary_levels.csv, 4.detailed_diagnostic_results_levels.csv, and 4.diagnostic_results_levels.RData\n")