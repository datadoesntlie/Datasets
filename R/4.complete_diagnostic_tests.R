...
cat("=== COMPREHENSIVE DIAGNOSTIC TESTING ===\n\n")
...
for (dep_var in dependent_vars) {
  ...
  re_model <- plm(model_formula, data = model_data, model = "random")
  ...
  model_tests$hausman <- hausman_test
  model_tests$preferred_model <- preferred_model
  
  cat("B. F-test for Fixed Effects vs Pooled:\n")
  pf_test <- pFtest(fe_model, pooled_model)
  print(pf_test)
  model_tests$pf_test <- pf_test
  
  cat("C. Breusch-Pagan LM test (RE vs Pooled):\n")
  ...
  complete_data <- model_data[complete.cases(model_data[, c("d_FLFP", "CPR", "TFR", 
                                                           "d_GDP_per_capita", "d_Life_expectancy_65", 
                                                           "d_Female_tertiary_education", "d_Urban_rate")]), ]
  
  if(nrow(complete_data) > 20) {
    corr_matrix <- cor(complete_data[, c("d_FLFP", "CPR", "TFR", "d_GDP_per_capita", 
  ...
  diagnostic_results[[dep_var]] <- model_tests
  
  cat("\n", rep("=", 80), "\n")
}

cat("\n=== DIAGNOSTIC SUMMARY ACROSS ALL MODELS ===\n")
...
for (dep_var in names(diagnostic_results)) {
  tests <- diagnostic_results[[dep_var]]
...
write_csv(summary_df, "diagnostic_summary.csv")
save(diagnostic_results, file = "diagnostic_results.RData")
...
for (model_name in names(diagnostic_results)) {
  tests <- diagnostic_results[[model_name]]
  ...
  if("pf_test" %in% names(tests)) {
    pf <- tests$pf_test
    cat("F-test (FE vs Pooled): F =", round(pf$statistic, 4), ", p =", round(pf$p.value, 6), "\n")
    
    detailed_results <- rbind(detailed_results, data.frame(
      Model = model_name,
      Test_Type = "Model Selection", 
      Test_Name = "F-test (FE vs Pooled)",
      Statistic = pf$statistic,
      P_Value = pf$p.value,
      Decision = ifelse(pf$p.value < 0.05, "Fixed Effects", "Pooled")
    ))
  }
...
write_csv(detailed_results, "diagnostic_results.csv")

cat("\n=== OVERALL RECOMMENDATIONS ===\n")
cat("1. Use preferred model specification (FE or RE) based on Hausman tests\n")
...
cat("4. Monitor multicollinearity issues in variable interpretation\n")
cat("5. Results saved to: diagnostic_summary.csv, detailed_diagnostic_results.csv, and diagnostic_results.RData\n")